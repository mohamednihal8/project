...
def generate_history_pdf(predictions):
    ...
    # Get PDF content as bytes and write to BytesIO buffer
...
def history():
    ...
    conn.close()
    # Parse the stringified inputs into dictionaries
    predictions = []
    for pred in raw_predictions:
        prediction_dict = {
            ...
            'user_id': pred[1],
            'model_type': pred[2],
            'inputs': ast.literal_eval(pred[3]),  # Safely parse the string into a dictionary
            'prediction': pred[4],
            'timestamp': pred[5]
        }
